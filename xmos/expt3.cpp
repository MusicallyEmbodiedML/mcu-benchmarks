...
int main(void) {
  ...
  printf("\n");
	...
	bfp_s32_t bfp_X, bfp_Y;

	int calculate_headroom=1;
	bfp_s32_init(&bfp_X, Xi, exp, LEN, calculate_headroom);
	bfp_s32_init(&bfp_Y, Yi, exp, LEN, calculate_headroom);

	now = hwtimer_get_time(timer);
	
	float_s64_t dotres;
	float_s32_t totals32 = f32_to_float_s32(0.0f);
	for(size_t i=0; i < iterations; i++) {
		dotres = bfp_s32_dot(&bfp_X, &bfp_Y);
		auto dotresf32 = float_s64_to_float_s32(dotres);
		totals32 = float_s32_add(totals32, dotresf32);
	}
	t2 = hwtimer_get_time(timer);
	// auto dotres32 = float_s64_to_float_s32(dotres);
	auto dotresf = float_s32_to_float(totals32);
	printf("Res 2: %f\n", dotresf);
	printf("%d start time: %lf\n", id, now * 1e-8);
	printf("%d end time: %lf\n", id, t2 * 1e-8);
	printf("%d total time: %lf\n", id, (t2-now) * 1e-8);
